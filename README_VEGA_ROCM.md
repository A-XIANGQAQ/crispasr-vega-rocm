...
# 编译
cd <无空格的构建路径>
.\build-hip-therock.ps1
```

### 4. 编译 Shim DLL / Build Shim DLL
...
├── build-hip-therock.ps1      # TheRock SDK 编译脚本 / TheRock build script
├── build-hip.bat              # 通用 HIP 编译脚本 / Generic HIP build script
├── bin-hip/                   # 运行目录 (DLL 需单独获取) / Runtime dir
│   ├── crispasr.exe           # 主程序 / Main binary
│   ├── ggml-hip.dll           # HIP 后端 (code object v4) / HIP backend
│   ├── amdhip64_7.dll         # ABI Shim (7.x → 5.7) / ABI bridge
│   ├── amdhip64.dll           # ROCm 5.7 运行时 / ROCm 5.7 runtime
│   ├── amd_comgr.dll          # ROCm 5.7 代码对象管理器 / Code object manager
│   ├── rocblas.dll            # rocBLAS (含 gfx900 内核) / rocBLAS with gfx900 kernels
│   ├── hipblas.dll            # hipBLAS
│   └── run_crispasr_hip.bat   # 运行脚本 / Run script
├── ggml/                      # ggml 源码 (含 HIP 后端) / ggml source with HIP backend
├── src/                       # CrispASR 源码 / CrispASR source
└── samples/                   # 测试音频 / Test audio
```
...
CrispASR 源码遵循其原始许可证（见 [LICENSE](LICENSE)）。
Shim DLL 代码（`hip-shim/`）采用 MIT 许可证，可自由使用。

CrispASR source code follows its original license (see [LICENSE](LICENSE)).
Shim DLL code (`hip-shim/`) is released under the MIT License and is free to use.
