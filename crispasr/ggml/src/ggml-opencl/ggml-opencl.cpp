...
static ggml_backend_opencl_context * ggml_cl2_init(ggml_backend_dev_t dev) {
    ...
    if (strstr(dev_ctx->device_name.c_str(), "Adreno") ||
        ...
        backend_ctx->gpu_family = GPU_FAMILY::INTEL;
               ...
               strstr(dev_ctx->platform_name.c_str(), "AMD") ||
               strstr(dev_ctx->platform_name.c_str(), "Advanced Micro Devices")) {
        // AMD desktop/mobile GPUs (Vega, RDNA, etc.) use wave64, same as Adreno
        // Treat them as ADRENO family for kernel compatibility
        GGML_LOG_INFO("ggml_opencl: AMD GPU detected: %s (using wave64/Adreno kernels)\n", dev_ctx->device_name.c_str());
        backend_ctx->gpu_family = GPU_FAMILY::ADRENO;
        ...
        // NVIDIA uses wave32 (warp size), treat like Intel
        GGML_LOG_INFO("ggml_opencl: NVIDIA GPU detected: %s (using wave32/Intel kernels)\n", dev_ctx->device_name.c_str());
        backend_ctx->gpu_family = GPU_FAMILY::INTEL;
    } else {
        // Unknown GPU: fallback to wave64 (AMD/Adreno compatible) instead of failing
        GGML_LOG_WARN("ggml_opencl: Unknown GPU '%s', falling back to wave64 mode\n", dev_ctx->device_name.c_str());
        backend_ctx->gpu_family = GPU_FAMILY::ADRENO;
        backend_ctx->adreno_gen = ADRENO_GPU_GEN::ADRENO_UNKNOWN;
        backend_ctx->adreno_wave_size = 64;
    }
    ...
    if (backend_ctx->gpu_family != GPU_FAMILY::ADRENO) {
        GGML_LOG_WARN("ggml_opencl: Adreno-specific kernels enabled but GPU reports as non-Adreno; wave32 kernels selected.\n");
    }
#endif
...
static ggml_backend_buffer_t ggml_backend_opencl_buffer_type_alloc_buffer(ggml_backend_buffer_type_t buffer_type, size_t size) {
    ...
    // Large buffer extension is Qualcomm Adreno-specific; skip on other GPUs
