...
static void solve_tri_f32_cublas(ggml_backend_cuda_context & ctx,
    ...
    // Vega (gfx900) 防御：rocBLAS 不可用时 cublas_handle 返回 nullptr，
    // 此处给出明确错误而非对 nullptr 调用 cublasSetStream 导致崩溃。
    cublasHandle_t handle = ctx.cublas_handle(id);
    if (handle == nullptr) {
        GGML_ABORT("solve_tri: cublas handle unavailable (rocBLAS disabled on this GPU)");
    }
    ...
    CUBLAS_CHECK(cublasStrsmBatched(handle, CUBLAS_SIDE_RIGHT, CUBLAS_FILL_MODE_UPPER, CUBLAS_OP_N,
