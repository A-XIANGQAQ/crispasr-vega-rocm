...
// Stringification macros — works even when the compiler wrapper (hipcc.bat)
// strips quotes from -D definitions. Converts bare macro values to string literals.
#define GGML_STR_HELPER(x) #x
#define GGML_STR(x) GGML_STR_HELPER(x)

const char * ggml_version(void) {
    return GGML_STR(GGML_VERSION);
}

const char * ggml_commit(void) {
    return GGML_STR(GGML_COMMIT);
}
